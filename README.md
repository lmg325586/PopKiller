...
| **隐私声明** | 设置 → 关于 内置隐私声明页，说明本地存储与联网行为 |
| **主页导航** | 卡片式主页：拦截引擎状态（含关闭时快捷开启）、并排的拦截/设置入口、系统信息卡片 |
| **系统托盘** | 最小化到托盘，右键菜单快速切换拦截状态、跳转页面 |
| **关闭行为** | 关闭窗口时选择：每次询问 / 完全退出 / 最小化到托盘 |
| **日志系统** | 实时刷新拦截日志，支持搜索、过滤与详细日志模式，记录焦点窃取等事件 |
...
- **拦截引擎**：显示引擎状态（运行中 / 已暂停 / 已关闭），关闭时提供「开启拦截」快捷按钮；
- **弹窗拦截 / 应用设置**：两张并排的操作卡，点击分别进入规则管理页与设置页；
- **系统信息**：显示 Windows 版本、版本号、注册用户与组织。

鼠标悬停卡片时显示辉光效果。也可通过左侧导航栏或托盘菜单切换页面。
...
> 冲突处理：如果添加的规则与已有相反名单规则内容相同，**不会直接新增**，而是弹出确认框询问是否编辑该冲突规则——选择「编辑该规则」会定位并打开该规则的编辑框（同时重置上方添加栏），选择「取消」则放弃。

### 编辑规则

1. 在规则列表中选中要修改的规则（也可右键或双击该规则）；
2. 弹出「编辑规则」对话框，回填该规则的名单类型 / 匹配字段 / 匹配模式 / 模式串；
3. 修改后点击「保存」完成更新，点击「取消」放弃；
...
- 通知显示被拦截的进程名与窗口标题；
- 通知内的「查看日志 / 加入白名单」按钮可直接操作（打开日志页 / 将该进程加入白名单）；
...
| 工作负载 | 使用 C++ 的桌面开发 |
| SDK | Windows App SDK 2.4（NuGet 还原） |
| 语言标准 | C++17 及以上 |
...
└── winui/                         # 主项目
    ...
    ├── HomePage.xaml(.cpp/.h)     # 主页（引擎状态、导航卡片、系统信息、辉光效果）
